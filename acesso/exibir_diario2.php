<script language="javascript" >
 window.onbeforeunload = function() {
    if( document.forms[0].btSalvou.value != 't'){
       return ' ATENÇÃO: Antes de fechar ou sair da pagina certifique-se que clicou no botão GRAVAR, caso contrário as atualizações de frequências não serão salvas.';
     }
 }
 </script>
 
<form action="exibir_diario2.php" method="POST">
<input  OnClick="  document.form[0].btSalvou.value = 't';  " type='submit' name='gravar' value=' Gravar '  />
<input name='btSalvou' id='btSalvou' value='f' type='hidden' >
...
$sql = mysql_query("SELECT distinct matricula,nome FROM v_aluno_disc WHERE turma_disc = $turma_d ORDER BY nome");
...
if ($count == 0) {
	...
	$sql1 = mysql_query("SELECT * FROM ced_view_alun_disc WHERE turma_disc = $turma_d ORDER BY nome");
	...
	<tr>
		...
		while ($ch_contar2 <= $ch_disciplina&&$ch_contar2 <=40) {
			...
			$campo_falta = $ch_contar2."_".$turma_d."_".$codigo;
			
			//verifica se data foi lançada
			...
			if($mes_aula == "--"){
				echo "
				<td align=\"center\"><div class=\"table_div1\"><b>.</b></font></div></td>";

			} else {
				echo "
				<td align=\"center\"><div class=\"table_div1\"><b><input type=\"checkbox\" $exibir_falta name=\"$campo_falta\"/></b></font></div></td>";
			}
