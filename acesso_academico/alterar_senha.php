...
$sql_senha = mysql_query("SELECT * FROM users WHERE usuario = '$usuario_ativo' AND senha = '$senha_atual'");
$sql_verificar = mysql_num_rows($sql_senha);

if($sql_verificar == 1){
	if($nova_senha == $c_nova_senha){
		$sql_att_senha = mysql_query("UPDATE users SET senha = '$nova_senha' WHERE usuario = '$usuario_ativo' AND senha = '$senha_atual'");
