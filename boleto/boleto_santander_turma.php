...
$sql_alunos = mysql_query("SELECT distinct matricula FROM ced_turma_aluno  WHERE id_turma = $id_turma LIMIT 2");
while($dados_t_alunos = mysql_fetch_array($sql_alunos)){
	$cod_aluno = $dados_t_alunos["matricula"];
	$cliente = $cod_aluno;
	$parcela = 1;
	$sql_rematricula = mysql_query("SELECT * FROM titulos WHERE cliente_fornecedor LIKE '$cliente' AND documento_fiscal LIKE '%REM%' LIMIT 1");
	$dados_rematricula = mysql_fetch_array($sql_rematricula);
	$codigo = $dados_rematricula["id_titulo"];
?>
...
if($total == 1) {
	...
	if($dias_atraso >=12&&$pago == ""&&$conta <> 'B00CB'&&$conta_lt<>"LT"){
	...
	if($dados["vencimento"] < $atual&&$recebido ==0&&$dias_atraso >=1){
		$juros2 = (($dados["valor"]+$juros1+$juros2)*0.0023)*$dias_atraso;
		mysql_query("UPDATE titulos SET juros2 = '$juros2' WHERE id_titulo LIKE '$codigo'");
		
		}
		
	//JUROS DE 5% APÓS 5 DIAS DE ATRASO / MODIFICADO PARA 10%
	if($dados["vencimento"] < $atual&&$dias_atraso >= 12&&$juros3 == 0&&$recebido ==0){
		$juros3 = ($dados["valor"]+$juros1+$juros2)*0.10;
		mysql_query("UPDATE titulos SET juros3 = '$juros3' WHERE id_titulo LIKE '$codigo'");
		}
		
	//JUROS DE 10% APÓS 30 DIAS DE ATRASO
	/*
	if($dados["vencimento"] < $atual&&$dias_atraso >= 30&&$juros4 == 0&&$recebido ==0){
	...
	*/
	...
	if($dados["vencimento"] < $atual&&$dias_atraso >=1&&$juros1 == 0&&$recebido ==0){
		$juros1 = ($dados["valor"])*0.02;
		$descfinal = $dados["descricao"]." | EM ATRASO | MULTA GERADA PELO SISTEMA";
...
$endereco_aluno = $dadosaluno["endereco"];
$cidade_aluno = $dadosaluno["cidade"];
$bairro_aluno = $dadosaluno["bairro"];
$cep_aluno = $dadosaluno["cep"];
...
$dadosboleto["endereco1"] = "Endereco :  $endereco_aluno / $bairro_aluno - $cidade_aluno";
$dadosboleto["endereco2"] = "CEP: $cep_aluno  ";
...
$dadosboleto["demonstrativo2"] = "Endereco :  $endereco_aluno / $bairro_aluno - $cidade_aluno";
...
require_once("include/funcoes_santander_banespa2.php");
require("include/layout_santander_banespa2.php");
echo "<div style=\"page-break-after:always\"></div>";//quebra de pagina
