...
if($total == 1) {
	...
	if($dados["vencimento"] < $atual&&$dias >= 5&&$juros3 == 0&&$recebido ==0&&$ano == date("Y")){
	...
	if($dados["vencimento"] < $atual&&$dias >= 30&&$juros4 == 0&&$recebido ==0&&$ano == date("Y")){
	...
	if($dados["vencimento"] < $atual&&$dias >=1&&$juros1 == 0&&$recebido ==0&&$ano == date("Y")){
...
$valor = $dados["valor"] + $dados["juros1"] + $dados["juros2"]+ $dados["juros3"] + $dados["juros4"] +$dados["acrescimo"] - (($dados["valor"]*$dados["desconto"])/100);
...
$dadosboleto["demonstrativo2"] = "Acréscimo - R$ ".number_format($acrescimo, 2, ',', '')."<br>Desconto - R$ ".number_format($desconto, 2, ',', '');
